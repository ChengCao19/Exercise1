...
%   region_index_sum:记录每个类别中的像素点的值的和
%   region_index_n:记录每个类别中的像素点的个数
...
for i=1:m
    for j=1:n
        if (pUnRegion(i,j)==0) % 如果当前点没有被处理
           ...
           % 更新该类别的和
           region_index_sum(region_index)=region_index_sum(region_index) +A(i,j);
           nStart = 1;
           nEnd = 1;
           ...
           while nStart<=nEnd  
              ...
              for k=1:4
                  ...
                  if((yy<=n)&(yy>=1)&(xx<=m)&(xx>=1)...
                     ...
                     pnGrowQueX(nEnd) = xx;
                     pnGrowQueY(nEnd) = yy;              
                     % 把象素(xx，yy)设置成region_index同时也表明该象素处理过
